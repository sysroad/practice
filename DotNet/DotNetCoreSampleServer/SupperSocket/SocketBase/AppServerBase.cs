...
namespace SuperSocket.SocketBase
{
        ...
        where TAppSession : AppSession<TAppSession, TRequestInfo>, IAppSession, new()
    {
        ...
        /// </summary>
        protected readonly TAppSession NullAppSession = default;
            ...
            foreach (var loader in m_CommandLoaders)
            {
                ...
                if (commands != null && commands.Any())
                {
                    foreach (var c in commands)
                    {
                        ...
                        if (!(c is ICommand<TAppSession, TRequestInfo> castedCommand))
                        {
                            if (Logger.IsErrorEnabled)
        ...
        {
            RootConfig = rootConfig ?? throw new ArgumentNullException("rootConfig");
        ...
        private bool SetupFinal()
        {
            ...
            if (!(Config is ServerConfig))
            {
                ...
                ServerConfig plainConfig = new ServerConfig(Config);
        ...
        {
            Bootstrap = bootstrap ?? throw new ArgumentNullException("bootstrap");
...
#pragma warning restore 0612, 618
        ...
        {
            if (!(m_SocketServer is IActiveConnector activeConnector))
